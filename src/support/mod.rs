...
impl Gl {
    pub fn draw_frame(&self, color: [f32; 4], triangles: &[Triangle]) {
        unsafe {
            self.gl.ClearColor(color[0], color[1], color[2], color[3]);
            self.gl.Clear(gl::COLOR_BUFFER_BIT);
            self.gl.BufferData(
                gl::ARRAY_BUFFER,
                (triangles.len() * mem::size_of::<Triangle>()) as gl::types::GLsizeiptr,
                triangles.as_ptr() as *const _,
                gl::STATIC_DRAW
            );
            self.gl.DrawArrays(gl::TRIANGLES, 0, (triangles.len() * 3) as i32);
        }
    }
}
